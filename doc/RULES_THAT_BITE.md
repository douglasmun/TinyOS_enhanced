# Rules that bite — full text

Relocated verbatim from `CLAUDE.md` on 2026-09-25 so the always-loaded file stays short.
`CLAUDE.md` keeps a one- or two-line version of each rule; this file keeps the reasoning.
Where another `doc/` file cites "CLAUDE.md's rule", the full text is here. Harness-writing
rules are also in `verify/CLAUDE.md`.

## Harness notes (from Build & run)
